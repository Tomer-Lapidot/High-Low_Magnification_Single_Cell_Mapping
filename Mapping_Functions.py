...
def Arrange_Tiles(_path_name, plot_location=False, n_well=-1):
    print('Arranging tiles from directory')

    warnings.filterwarnings('ignore')

    onlyfiles = [f for f in listdir(_path_name) if isfile(join(_path_name, f)) and join(_path_name, f).endswith('.nd2')]
    onlyfiles = np.array(onlyfiles, dtype=str)
    ...
    X = _Tile_List['x'].to_numpy()
    Y = _Tile_List['y'].to_numpy()
    P = _Tile_List['position'].to_numpy()
    mpp = _Tile_List['mpp'].iloc[0]

    X = (X - X.min()) / (mpp * w)
    Y = (Y - Y.min()) / (mpp * h)

    dim_M =np.max([int(Y.max() + 1), int(X.max() + 1)])
    _M = -1 * np.ones([int(dim_M), int(dim_M)], dtype=int)
    # M = -1 * np.ones([int(Y.max() + 1), int(X.max() + 1)], dtype=int)
    for k in P:
        _M[int(round(Y[k])), int(round(X[k]))] = k
...
def Check_Tile_Configuration(_path_name, _M, file_type='nd2', n_well = -1, crop_edges = 0, plot_align = False, print_config = False, plot_tiles = False):
    print('Verifying tiles align with tile-map')

    def get_last2d_old(data):
        if data.ndim <= 2:
            return data
        slc = [0] * (data.ndim - 2)
    ...
    for m in range(8): # tqdm(range(8)):
        ...
        if crop_edges > 0:
            ...
            img_0 = img_0[crop_edges:R_i - crop_edges, crop_edges:R_j - crop_edges]
            img_1 = img_1[crop_edges:R_i - crop_edges, crop_edges:R_j - crop_edges]
            img_2 = img_2[crop_edges:R_i - crop_edges, crop_edges:R_j - crop_edges]
            img_3 = img_3[crop_edges:R_i - crop_edges, crop_edges:R_j - crop_edges]
    ...
    if plot_tiles:
        ...
        print('Plotting tile adjacent to', int(_M_crt[MC_I, MC_J]), int(_M_crt[MC_I + 1, MC_J]), int(_M_crt[MC_I, MC_J + 1]), int(_M_crt[MC_I + 1, MC_J + 1]))
